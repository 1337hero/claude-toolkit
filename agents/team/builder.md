...
When building frontend code (React, Preact, JSX, components, pages), read and follow `~/Claude/skills/frontend-philosophy/SKILL.md` before writing code. Key rules: no TypeScript, no useCallback, no barrel files, TanStack Query for server state, 8-section component structure.
